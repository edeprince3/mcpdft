...
C
     ...
     *                 FTOL,STPMAX,MAXFEV,W,IFLAG,INFO)
C
      INTEGER N,M,IPRINT(2),IFLAG,MAXFEV,INFO
      DOUBLE PRECISION X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M)
      DOUBLE PRECISION F,EPS,XTOL,FTOL
      LOGICAL DIAGCO
C
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
      DOUBLE PRECISION GTOL,ONE,ZERO,GNORM,DDOT,STP1,STPMIN,
     .                 STPMAX,STP,YS,YY,SQ,YR,BETA,XNORM
      INTEGER MP,LP,ITER,NFUN,POINT,ISPT,IYPT,
...
C     *08/11/2006* NOW PASS FTOL AND MAXFEV AS PARAMETERS TO LBFGS
C     
C      FTOL= 1.0D-4
C      MAXFEV= 20
C
      IF(IPRINT(1).GE.0) CALL LB1(IPRINT,ITER,NFUN,
     *                     GNORM,N,M,X,F,G,STP,STPMAX,FINISH)
C
C    --------------------
...
C     ----------------------------------------------------
 ...
 172  CONTINUE
     ...
     *            XTOL,STPMAX,MAXFEV,INFO,NFEV,DIAG)
      IF (INFO .EQ. -1) THEN
        IFLAG=1
        RETURN
      ENDIF
...
C
      BLOCK DATA LB2
      INTEGER LP,MP
      DOUBLE PRECISION GTOL,STPMIN
      COMMON /LB3/MP,LP,GTOL,STPMIN
      DATA MP,LP,GTOL,STPMIN/6,-1,9.0D-01,1.0D-30/
      END
C
C
C   ----------------------------------------------------------
...
c
      ...
      integers i,incx,incy,ix,iy,m,mp1,n
c
      if(n.le.0)return
...
C     **************************
C
      SUBROUTINE MCSRCH(N,X,F,G,S,STP,FTOL,XTOL,STPMAX,MAXFEV,INFO,
     *                  NFEV,WA)
